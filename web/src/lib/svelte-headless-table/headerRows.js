    ...
    cells.map((cell, columnIdx) => {
      ...
      if (rowIdx === maxHeight - 1)
        return new FlatDisplayHeaderCell({
          id: columnIdx.toString(),
          colstart: columnIdx,
        })
      const flatId =
        rowMatrix[maxHeight - 1][columnIdx]?.id ?? columnIdx.toString()
...
export const getOrderedColumnMatrix = (columnMatrix, flatColumnIds) => {
  ...
  flatColumnIds.forEach((key, columnIdx) => {
    const nextColumn = columnMatrix.find((columnCells) => {
      ...
      if (!flatCell.isFlat()) {
        throw new Error(
          "The last element of each column must be a `FlatHeaderCell`",
        )
      }
