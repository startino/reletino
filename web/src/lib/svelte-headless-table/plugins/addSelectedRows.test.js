...
test("reading rows selected store", () => {
  ...
  const vm = table.createViewModel(columns)
  const { selectedDataIds, allRowsSelected, someRowsSelected } =
    vm.pluginStates.select
...
test("reading page rows selected store", () => {
  ...
  const vm = table.createViewModel(columns)
  const { selectedDataIds, allPageRowsSelected, somePageRowsSelected } =
    vm.pluginStates.select
