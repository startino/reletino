...
const isSomeSubRowsSelectedForRow = (
  row,
  $selectedDataIds,
  linkDataSubRows,
) => {
...
const getRowIsSelectedStore = (row, selectedDataIds, linkDataSubRows) => {
  ...
  const update = (fn) => {
    selectedDataIds.update(($selectedDataIds) => {
      const oldValue = isAllSubRowsSelectedForRow(
        row,
        $selectedDataIds,
        linkDataSubRows,
      )
      const $updatedSelectedDataIds = { ...$selectedDataIds }
      writeSelectedDataIds(
        row,
        fn(oldValue),
      ...
      if (row.parentRow !== undefined && row.parentRow.isData()) {
        $updatedSelectedDataIds[row.parentRow.dataId] =
          isAllSubRowsSelectedForRow(
            row.parentRow,
            $updatedSelectedDataIds,
            linkDataSubRows,
          )
...
export const addSelectedRows =
  ...
  ({ tableState }) => {
    ...
    const getRowState = (row) => {
      const isSelected = getRowIsSelectedStore(
        row,
        selectedDataIds,
        linkDataSubRows,
      )
      ...
      const isAllSubRowsSelected = derived(
        selectedDataIds,
        ($selectedDataIds) => {
          return isAllSubRowsSelectedForRow(
            row,
            $selectedDataIds,
            linkDataSubRows,
          )
