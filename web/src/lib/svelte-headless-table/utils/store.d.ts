...
export type WritableKeys<T> = {
  [K in keyof T]: T[K] extends undefined
    ? Writable<T[K] | undefined>
    : Writable<T[K]>
}
export type ReadableKeys<T> = {
  [K in keyof T]: T[K] extends undefined
    ? Readable<T[K] | undefined>
    : Readable<T[K]>
}
export type ReadOrWritableKeys<T> = {
  [K in keyof T]: T[K] extends undefined
    ? ReadOrWritable<T[K] | undefined>
    : ReadOrWritable<T[K]>
}
...
export interface RecordSetStore<T extends string | number>
  extends Writable<Record<T, boolean>> {
  toggle: (item: T) => void
  add: (item: T) => void
  addAll: (items: T[]) => void
  remove: (item: T) => void
