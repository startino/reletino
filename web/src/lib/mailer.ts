...
export const sendAdminEmail = async ({ subject, body }: { subject: string; body: string }) => {
...
export const sendUserEmail = async ({
	...
	const { data: serviceUserData } = await serverSupabase.auth.admin.getUserById(user.id);
	const emailVerified =
...
export const sendTemplatedEmail = async ({
	...
	try {
		const emailTemplate = await import(`./emails/${template_name}_text.svelte`).then(
			(mod) => mod.default
		);
	...
	try {
		const emailTemplate = await import(`./emails/${template_name}_html.svelte`).then(
			(mod) => mod.default
		);
		const { html } = emailTemplate.render(template_properties);
		htmlBody = html;
	...
	if (!plaintextBody && !htmlBody) {
		console.log('No email body: requires plaintextBody or htmlBody. Template: ', template_name);
		return;
	}
