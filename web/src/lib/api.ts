...
import {
  PUBLIC_REDDIT_USERNAME,
  PUBLIC_REDDIT_PASSWORD,
  PUBLIC_BASE_URL,
...
export const publishComment = async (
  ...
  const success: boolean = await fetch(`${PUBLIC_BASE_URL}/publish-comment`, {
    method: "POST",
    headers: {
    ...
    .then(async (response) => {
      if (!response.ok) {
        console.error(
          `Failed to publish comment. Bad response`,
          response,
          await response.json()
        );
        return false;
...
export const generateComment = async (
  ...
  const comment: string = await fetch(`${PUBLIC_BASE_URL}/generate-comment`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    ...
    .then(async (response) => {
      if (!response.ok) {
        console.error(
          `Failed to generate comment. Bad response`,
          response,
          await response.json()
        );
        return "";
...
export const markAsIrrelevant = async (
  ...
  const success: boolean = await fetch(
    `${PUBLIC_BASE_URL}/mark-lead-as-irrelevant`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
