<script lang="ts">
  ...
  export let data: {
    relevantSubmissions: Tables<"submissions">[]
  ...
  let selectedProject = projects[0]
    ? { value: projects[0].id, label: projects[0].title }
  ...
  $: relevantSubmissions = relevantSubmissions.filter(
    (row) => row.project_id === selectedProject.value,
  )
  ...
  <Resizable.Pane defaultSize={50}>
    <div class="flex flex-col p-6">
      ...
      {#key relevantSubmissions}
        <DataTable
          bind:activesubmission={activeSubmission}
